...
if(!isset($SEARCH_SHORTCODE)) 
{
  ...
  $SEARCH_SHORTCODE = "<div class='input-group stylish-input-group'>
                         <input class='form-control' placeholder='".LAN_THEME_10."' type='text' name='q'>
	...
	<input class='btn btn-default button search' type='submit' name='s' value=\"<i class='fa fa-search'></i>\" />";
  */
...
<form class="form-horizontal" role="form" id="searchform" method="get" action="{SEARCH_FORM_URL}">
  <div class="form-group">
    <label for="inputEmail3" class="col-sm-3 control-label">'.LAN_199.'</label>
	  <div class="col-sm-9">{SEARCH_MAIN}</div>
  </div>
  <div id="search-enhanced" {ENHANCED_DISPLAY}>{SEARCH_ENHANCED}</div>
';

$SEARCH_TEMPLATE['form']['advanced'] = '
	<div class="form-group">
	  <label for="inputPassword3" class="col-sm-3 control-label">{SEARCH_ADV_A}</label>
	  <div class="col-sm-9">{SEARCH_ADV_B}</div>
  </div>
';
...
$SEARCH_TEMPLATE['form']['enhanced'] = '
	<div id="{ENHANCED_DISPLAY_ID}" class="form-group">
	  ...
	  <div class="col-sm-9">{ENHANCED_FIELD}</div>
  </div>
';
...
$SEARCH_TEMPLATE['form']['category'] = '
	<div class="form-group">
	  ...
	  <div class="col-sm-9">{SEARCH_MAIN_CHECKBOXES}{SEARCH_DROPDOWN}&nbsp;</div>
	</div>
	{SEARCH_ADVANCED}
';
